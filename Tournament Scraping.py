...
#     tournament_element = soup.find(style="class: 'stable'")  # search for tournament list
    tourney_links = soup.find_all('a', attrs={'href': re.compile("^http://")})  # get all the tournament links
    tournament_link = []

    for element in tourney_links:
        if 'event' in element.text:
            tournament_link.append(element['value'])  # link to decks of that tournament
...
def get_deck_links_tournament(tournament_url: str) -> pd.DataFrame:
    ...
    decks_df = pd.DataFrame(decks, columns=['Rank', 'Name', 'Deck link', 'Author name'])
    return decks_df
def get_deck_links_all(mtg_format: str) -> pd.DataFrame:
    ...
    for index, row in tier_lists.iterrows():
        deck_from_tournament = get_deck_links_tournament(row['Link to month'])
